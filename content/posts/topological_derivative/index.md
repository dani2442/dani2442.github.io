...
(§3), the discretization (§4), and the algorithm and examples (§5–§6); the
appendices hold the exterior solution, the numerical verification of the
formula and an exact check. All the code is in
...
| $\ell$ | Scalar multiplier for the material constraint in §5; distinct from the load functional $\ell[v]$. |
...
plane strain the factor $1-\nu^2$ survives. A reader who knows the literature
may balk here, since the standard statement of this result carries the
conspicuous factors $1-3\nu$ and $1-4\nu$. Those belong to the mixed
stress--strain (polarization-tensor) form, which is the same number written
differently;
[Appendix D](#appendix-d-equivalent-forms-and-the-polarization-tensor) carries
out the conversion and disposes of a frequently seen paraphrase that mixes the
two forms.
...
### 3.5 What the remainder estimate requires
...
because they are exactly what the verification in
[Appendix B](#appendix-b-numerical-verification-of-the-formula) measures.
...
*The raw $D_TJ$ before filtering: dark regions are expensive to perforate,
pale regions are inexpensive.*

## 5. From the gradient to an algorithm

### 5.1 The constrained problem and its topological optimality condition

Fix a **hold-all** design domain $\Omega_0$ containing $\Gamma_D$ and
$\Gamma_N$, and solve
$$\min_{\Omega\subset\Omega_0}\ J(\Omega)
\qquad\text{subject to}\qquad
|\Omega|\le V\,|\Omega_0| .\tag{5.1}$$
...
=\pi\varepsilon^2\big(D_TJ(\hat x)-\ell\big)+o(\varepsilon^2),\tag{5.2}$$
since the hole removes area $\pi\varepsilon^2$. So the **first-order
topological optimality condition** for (5.1) is
$$D_TJ(\hat x)\ \ge\ \ell\qquad\text{for a.e. }\hat x\in\Omega,\tag{5.3}$$
...
Condition (5.3) is what turns the formula into an algorithm: given a field
...
approximation: problem (5.1) has, in general, **no minimizer** — minimizing
...
The full-material compliance is $J_0=38.729$; the final design has $J=73.61$.
The animation at the top of the post follows every iteration.

### 6.2 A three-pier bridge
...
and never on ersatz. Unlike the clamped cantilever, the piers constrain
individual mesh nodes: these are discrete pin idealizations, not the
positive-length clamped boundaries covered by Theorem 1.1.

The full-material compliance is $J_0=1.712$; after $120$ updates $J=2.232$.
...
the state solved on it.*

### 6.3 How the bridge changes with the parameters
...
The full compliance and volume histories, along with the parameters of every
run, are available in [the numerical results](td_results.json).

### 6.4 The same span, hanging
...
the two pier heads stay solid ($366$ cells).

The full-material compliance is $J_0=4.237$; the final design has $J=6.572$.
...
Repeating the sweep of §6.3 over the same nine parameter pairs:
...
### 6.5 The same bridge, upside down

Keep the load on the bottom edge as in §6.4, but put **three** piers back — this
time along the **top** edge, at $x=0$, $1.5$ and $3$. The deck now hangs from
three points above it instead of standing on three below. Two deck rows and the
three pier heads stay solid, exactly as in §6.2.
...
over the last tenth of its length up to the right edge. The traction is normal
to the loaded face here rather than tangential to it. The $120\times120$
...
with $r_{\min}/h=3.5$ and $90$ updates. The re-entrant corner is a stress
singularity in the initial domain: as §3.5 explains, the small-hole expansion
must be used away from such boundary points.
...
(3.2) plus the exterior solution (3.8), and
[Appendix B](#appendix-b-numerical-verification-of-the-formula) confirms it
against meshed holes for both plane models and three remote stress states,
with the equivalent algebraic forms of
[Appendix D](#appendix-d-equivalent-forms-and-the-polarization-tensor)
cross-checked over four Poisson ratios. The
...
- *Insertion is heuristic*, as discussed in §5.2.
- *Tension and compression are indistinguishable.* Both $J$ and $D_TJ$ are
  ...
  §6.5 shows the consequence: the suspended deck is the arch viaduct of §6.2
...
coefficients of Proposition D.1.)
...
polarization-tensor form (D.1)–(D.3) and the plane-strain factor $1-4\nu$.)
...
optimality condition (5.3) used as a fixed-point iteration on the level set.)
...
ill-posedness of (5.1) without a length scale.)
...
two-iteration averaging — the update of §5.2.)
...
## Appendix B: numerical verification of the formula
...
*exactly*, so
$$J(\Omega_0)=\sigma^\infty:\varepsilon^\infty\,|\Omega_0|\tag{B.1}$$
...
$J(\Omega_a)$, and
$$\widehat{D_TJ}:=\frac{J(\Omega_a)-J(\Omega_0)}{|B_a|}\tag{B.2}$$
...
[Appendix C](#appendix-c-the-annulus-solution-and-an-exact-check)),
...
## Appendix C: the annulus solution and an exact check

The biaxial case of the verification above can be done in closed form, which is
what makes the last row of its table a genuine check rather than a second
numerical experiment.

Take the *full* annulus $a\le r\le R$ in plane stress, traction free at $r=a$
and loaded by $\sigma_{rr}=p$ at $r=R$. (The computation there uses a quarter
...
conditions give
$$B=-Aa^2,\qquad A=\frac{p}{1-a^2/R^2}.\tag{C.1}$$
...
which is (B.3). Dividing by $|B_a|=\pi a^2$,
...
## Appendix D: equivalent forms and the polarization tensor
...
$$D_TJ(\hat x)=\mathbb P\,\hat\sigma:\hat\varepsilon .\tag{D.1}$$
...
> **Proposition D.1.** The polarization form of (3.10) is
...
which mixes the two: it takes the coefficients of (D.2) and attaches them to
...
script `validate.py` checks (3.10), the principal-stress form and (D.2)–(D.3)
